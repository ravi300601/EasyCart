...
function Page1({handleNextPage, name, address, pincode, phoneNumber, setName, setAddress, setPincode, setPhoneNumber }) {
    return (
        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form className="space-y-4 md:space-y-6" action="#">
                <div>
                    ...
                    <input type="name" value={name} onChange={(e) => setName(e.target.value)} name="name" id="name" className=" border outline-0 border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 bg-gray-100" required />
                </div>
                <div>
            ...
            <button onClick={handleNextPage} type="button" className="focus:outline-none w-full text-white bg-slate-900 hover:bg-slate-900 outline-0 font-medium rounded-lg text-sm px-5 py-2.5 ">Order Now</button>
        </div>
    );
}


function Page2({handlePreviousPage, buyNow, closeModal, setPaymentMethod, grandTotal, paymentMethod}) {
    return (
        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 ms-0" onClick={handlePreviousPage}>
            ...
            <form className="space-y-4 md:space-y-6" action="#">
                ...
                <div class="flex items-center">
                    ...
                    <label for="mastercard" class="text-gray-700">Cash on Delivery</label>
                    <div class="ml-2">
                        <p class="text-sm text-gray-500">Pay at your doorstep</p>
                    </div>
                    <input type="radio" id="cod" name="payment"  value='cod' onChange={(e) => setPaymentMethod(e.target.value)} class="ms-auto border-gray-300 text-indigo-600 focus:ring-blue-500 focus:border-blue-500 rounded-full"/>
                </div>
            </form>
            { paymentMethod === 'gpay' ? (
                <GooglePayButton
                ...
                <button onClick={() => {buyNow(); closeModal()}} type="button" className="focus:outline-none w-full text-white bg-slate-900 hover:bg-slate-800 outline-0 rounded-lg text-sm font-semibold px-5 py-2.5 ">Buy with {paymentMethod.toUpperCase()}</button>
            )}
        </div> 
    );
}
